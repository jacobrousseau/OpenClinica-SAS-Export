...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:odm="http://www.cdisc.org/ns/odm/v1.3" xmlns:OpenClinica="http://www.openclinica.org/ns/odm_ext_v130/v3.1" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" version="2.0" xsi:schemaLocation="http://www.cdisc.org/ns/odm/v1.3 OpenClinica-ODM1-3-0-OC3-0.xsd">
    ...
    <xsl:key name="item-datatype" match="odm:ItemDef" use="@DataType"/>
    ...
    <xsl:variable name="sas_typemap">
        <row oc="date">
            ...
            <INFORMAT width="10">E8601DA</INFORMAT>
        </row>
        <row oc="time">
            <TYPE>numeric</TYPE>
            <DATATYPE>time</DATATYPE>
            <FORMAT>$HHMM.5</FORMAT>
            <INFORMAT>$HHMM.5</INFORMAT>
        </row>
        <row oc="float">
            <TYPE>numeric</TYPE>
    ...
    <xsl:template match="odm:ItemRef" mode="map">
        ...
        <xsl:element name="COLUMN">
            ...
            <xsl:choose>
                <!-- We consider all items with a name containing 'Time' as having the type time -->
                <xsl:when test="contains(upper-case($itemdef/@Name), 'TIME')">
                    <xsl:copy-of copy-namespaces="no" select="$typemap/row[@oc='time']/*"/>
                </xsl:when>
                <xsl:otherwise>
